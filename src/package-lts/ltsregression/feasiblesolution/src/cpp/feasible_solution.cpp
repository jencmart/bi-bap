...
void goThroughAllPairs(double & delta, int & iSwap, int & jSwap, const Eigen::MatrixXd & dataJX,
   ...
   //  exit(1);
    // Eigen::MatrixXd inversion = invers; // todo this is some lazy fix...
    ...
    for (unsigned i = 0; i < h; ++i) {
        for (unsigned j = 0; j < nMinusH; ++j) {
           ...
           // std::cout << "*********"  << std::endl;
           // std::cout<< dataJX(i, Eigen::all) * inversion * (dataJX(i, Eigen::all).transpose()) << std::endl;
           // std::cout << "*********"  << std::endl;
...
// *********************************************************************************************************************
Result * refinementProcess(std::vector<int> & indexesJ, std::vector<int> & indexesM, const Eigen::MatrixXd & X, const Eigen::MatrixXd & y ) {
