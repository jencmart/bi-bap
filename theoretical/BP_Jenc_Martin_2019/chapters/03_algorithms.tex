...
Finally if we multiply $\vec{Q}_\varphi(i, j) \vec{x} = \vec{p}$ we can see that $\vec{p}$ is same as vector $\vec{x}$ except $p_i$ and $p_j$ so that:
...
\begin{equation}
    \vec{Q}_{\varphi_e}\ldots\vec{Q}_{\varphi_2}\vec{Q}_{\varphi_q}\vec{A} = \vec{R}
\end{equation}
where $\vec{Q}_{\varphi_e}\ldots\vec{Q}_{\varphi_2}\vec{Q}_{\varphi_1}$ is actually $\vec{Q}^T$. So $\vec{Q}$ is obtained by 

\begin{equation}
 \vec{Q} = \vec{Q^T}_{\varphi_1}\vec{Q^T}_{\varphi_2}\ldots\vec{Q^T}_{\varphi_e}.
\end{equation}

We can see that for the each row of the matrix, we need one additional matrix $\vec{Q}_{\varphi_i}$, and with such matrix, we are multiply only two rows. Moreover, the rows are getting shorter after each finished column. So the total number of operations can be expressed as
...
This means that  $\vec{m_1}$ corresponds to $h$-element subset with smallest absolute residuals $r_{i}^2(\vec{w_0})$.
...
\begin{proof}
    ...
    Finally, we must compute $\hat{w}$ OLS estimate on a $h$-element subset of data. Because $h$ is linearly dependent on $n$, we can say that it is $\mathcal{O}(p^2n + p^3)$ which is asymptotically dominant against previous steps which are $\mathcal{O}(np + n)$. Because we assume$n \ge p$ then $\mathcal{O}(p^2n + p^3) \sim \mathcal{O}(p^2n) $
\end{proof}
...
We describe algorithms for updating the QR decomposition in the section \ref{differentcomputation}. 
...
So if set $H_p$ satisfies \ref{hp:set:condition} then it can be used for finding $\mathcal{H}_p$ subset, thus we would be able to use this set for the multidimensional version of the algorithm.
