...
\begin{algorithm}[H]
    ...
    \KwIn{$\vec{H}^{-1} \in \mathbb{R}^{p \times p}$, 
    ...
    \For{$m_i \in  O_m$}{  
        \For{$m_j \in  Z_m$}{ 
            ...
            $d_{i,i} = \vec{x_{m_i}} \vec{H}^{-1} \vec{x^T_{m_i}}$ \;
            $d_{i,j} = \vec{x_{m_i}} \vec{H}^{-1} \vec{x^T_{m_j}}$ \;
            $d_{j,j} = \vec{x_{m_j}} \vec{H}^{-1} \vec{x^T_{m_j}}$ \;
...
\begin{algorithm}[H]
    ...
    $l \gets 0$\;
       ...
       $\vec{H} = (\vec{X}^T\vec{M}^T\vec{X})$\;
       $\vec{H}^{-1} = calculate~inversion~of~\vec{H}$\;
...
% only to different computation - 1100
%%% IMPROVED by FSA %%%%%%%
