...
\begin{proof}
	...
	\todo{ CO zo toho je vic? Neni casove narocnejsi vynasobeni $\m{X^T}\m{X}$ nez inversion, kdyz bereme v uvahu $n >> p$ ???}
...
\begin{observation} \label{prandomsamples}
	...
	at least one $J_{1_i}$ such that its corresponding data samples does not contains outliers, goes to
	$$ 1-(1-(1-\epsilon)^h)^m  > 0$$
\end{observation}
...
\todo{pseudokod - tak moc nechci :-D}
...
\todo{presunout cholesky popis do 1. kapitoly}
\begin{definition}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Givens Rotation} \label{givensrotation}% NOT NUMBERED DUE TO *
...
% BLAS routine for matrix multiplication is implemented by \defterm{dgemm}
